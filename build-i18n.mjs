...
import { taxFreeAirportSteps, taxFreeComparison, taxFreeConsumablesCap, taxFreeConsumptionTax, taxFreeFaq, taxFreeItemCategories, taxFreePeriods, taxFreeRefundMayBeLess, taxFreeSeparateShipmentFrom, taxFreeSources, taxFreeVerifiedAt } from "./lib/tax-free-data.mjs";
...
function renderJapaneseBrandsGuide() {
  ...
  const cards = brands.map((brand, index) => `<article class="brand-guide"><p class="number">0${index + 1}</p><h2>${esc(brand.name)}</h2><p class="ja-name" lang="ja">${esc(brand.nameJa)}</p><p>${esc(brand.text)}</p>${(() => {
    // 英語版に商品ページがあるブランドだけ、Sillage内の導線を出す。
    // 商品が無いブランドへのリンクは作らない（存在しないページに送らないため）。
    const entry = brandEntries.find((b) => b.key === brand.name);
    if (!entry) return "";
    const url = brandHasDetail(entry.key) ? brandRouteEn(entry) : entry.products[0].routeEn;
    return `<p class="brand-guide-cta"><a href="${esc(url)}">${entry.products.length} ${esc(brand.name)} fragrance${entry.products.length === 1 ? "" : "s"} on Sillage &rarr;</a></p>`;
  })()}<p><a href="${esc(brand.href)}" target="_blank" rel="noopener noreferrer">Official site <span aria-hidden="true">↗</span></a>${brand.extra ? ` · <a href="${esc(brand.extra)}" target="_blank" rel="noopener noreferrer">Official global site <span aria-hidden="true">↗</span></a>` : ""}</p></article>`).join("");

...
function renderStoreCard(store, index = null) {
  ...
  return `<article class="shop" id="${esc(store.id)}"><p class="shop-number">${number}${esc(STORE_TYPE[store.storeType] || store.storeType)}</p><h3>${esc(store.nameEn)}</h3><p class="ja-name" lang="ja">${esc(store.nameJa)}</p><dl><div><dt>Area</dt><dd>${esc(store.area)}</dd></div><div><dt>Address</dt><dd>${esc(store.addressEn)}</dd></div><div><dt>Nearest station</dt><dd>${esc(store.nearestStation)}</dd></div>${hours}<div><dt>English support</dt><dd>${esc(englishSupportLabel(store.englishSupport))}</dd></div><div><dt>Tax-free shopping</dt><dd>${esc(taxFreeLabel(store.taxFree))}</dd></div></dl><div class="shop-links"><a data-store-link="official" data-store-id="${esc(store.id)}" data-store-city="${esc(store.city)}" href="${esc(store.officialUrl)}" target="_blank" rel="noopener noreferrer">Official information <span aria-hidden="true">↗</span></a><a data-store-link="maps" data-store-id="${esc(store.id)}" data-store-city="${esc(store.city)}" href="${esc(store.googleMapsUrl)}" target="_blank" rel="noopener noreferrer">Google Maps <span aria-hidden="true">↗</span></a></div></article>`;
}

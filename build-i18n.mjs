...
const kyotoShops = JSON.parse(readFileSync("data/kyoto-shops.json", "utf8")).kyoto_fragrance_shops;
const kyotoEnglish = JSON.parse(readFileSync("data/i18n/kyoto-shops.en.json", "utf8"));
const COUNTRY_NAMES = {
  AU: "Australia", FR: "France", IT: "Italy", UK: "United Kingdom", US: "United States",
};
const KYOTO_CATEGORY = {
  custom: "Custom fragrance",
  niche: "Niche & selection",
  department: "Department store & beauty floor",
  "kyoto-original": "Kyoto scent experience",
};
...
<main>
  ...
  <section class="section" id="paths"><div class="wrap"><p class="eyebrow">Explore Sillage</p><h2>Choose a practical way in.</h2><p class="lede">Browse a curated English catalogue, understand the brands, or plan where to test fragrance during a visit to Japan.</p><div class="cards">
    ...
    <a class="card path-card" href="/en/guides/best-japanese-perfume-brands/"><h3>Japanese fragrance brands</h3><p>Start with well-documented brands and learn what to look for before visiting a store in Japan.</p><small>Read the guide →</small></a>
  </div></div></section>
  <section class="section"><div class="wrap"><p class="eyebrow">Where to buy and try</p><h2>Plan fragrance shopping in Japan.</h2><div class="guide-pair"><a class="card" href="/en/guides/perfume-shopping-kyoto/"><h3>Perfume shopping in Kyoto</h3><p>Use a verified list of 18 custom-fragrance studios, niche stores and department-store beauty floors, grouped for travelers.</p><small>Open the Kyoto guide →</small></a><a class="card" href="/en/guides/best-japanese-perfume-brands/"><h3>Japanese perfume brands</h3><p>Learn how SHIRO, J-Scent, SHOLAYERED and AUX PARADIS approach fragrance, without treating the list as a ranking.</p><small>Explore Japanese brands →</small></a></div></div></section>
...
function renderKyotoGuide() {
  ...
  const headline = "Where to Buy Perfume in Kyoto: 18 Fragrance Shops";
  const description = "A traveler-focused guide to 18 verified fragrance shops in Kyoto, including custom fragrance studios, niche stores and department-store beauty floors.";
  const shops = kyotoShops.map((shop) => ({ ...shop, ...(kyotoEnglish.shops[shop.slug] || {}) }));
  const cards = shops.map((shop, index) => `<article class="shop" id="${esc(shop.slug)}"><p class="shop-number">${String(index + 1).padStart(2, "0")} · ${esc(KYOTO_CATEGORY[shop.category] || shop.category)}</p><h2>${esc(shop.nameEn)}</h2><p class="ja-name" lang="ja">${esc(shop.name)}</p><dl><div><dt>Area</dt><dd>${esc(shop.area)}</dd></div><div><dt>Address</dt><dd>${esc(shop.addressEn)}</dd></div><div><dt>Nearest station</dt><dd>${esc(shop.nearestStation)}</dd></div><div><dt>Opening hours</dt><dd>${esc(shop.hours)}${shop.closed ? `<br><small>Closed: ${esc(shop.closed)}</small>` : ""}</dd></div><div><dt>English support</dt><dd>Not confirmed</dd></div><div><dt>Tax-free shopping</dt><dd>Not confirmed</dd></div></dl><div class="shop-links"><a href="${esc(shop.officialUrl)}" target="_blank" rel="noopener noreferrer">Official information <span aria-hidden="true">↗</span></a><a href="${esc(shop.google_maps_url)}" target="_blank" rel="noopener noreferrer">Google Maps <span aria-hidden="true">↗</span></a></div></article>`).join("");
  const itemList = { "@context": "https://schema.org", "@type": "ItemList", numberOfItems: shops.length, itemListElement: shops.map((shop, index) => ({ "@type": "ListItem", position: index + 1, name: shop.nameEn, url: `${canonical}#${shop.slug}` })) };
  const article = articleLd({ headline, description, canonical, modified: kyotoEnglish.lastVerified });
  const breadcrumb = breadcrumbLd([{ name: "Sillage", url: `${SITE}/en/` }, { name: "Kyoto perfume shopping", url: canonical }]);
  return `<!DOCTYPE html><html lang="en"><head>${head({ title: `${headline} | Sillage`, description, canonical, jaPath: "/columns/kyoto-fragrance-shops", ogType: "article" })}<script type="application/ld+json">${json(article)}</script><script type="application/ld+json">${json(breadcrumb)}</script><script type="application/ld+json">${json(itemList)}</script>${sharedCss}<style>.guide-hero{padding:76px 0 58px}.guide-hero h1{font:500 clamp(40px,7vw,70px)/1.12 "Bodoni Moda",serif;margin:10px 0}.notice{border-left:2px solid var(--gold);padding:14px 18px;color:#b6b3ab;background:var(--panel);margin-top:30px}.shop-list{display:grid;grid-template-columns:repeat(2,1fr);gap:20px;padding-bottom:90px}.shop{background:var(--panel);border-top:2px solid var(--gold);padding:24px;min-width:0}.shop h2{font:500 25px "Bodoni Moda",serif;margin:4px 0}.shop-number{color:var(--gold);font-size:11px;text-transform:uppercase;letter-spacing:.8px}.shop dl{margin:22px 0}.shop dl div{display:grid;grid-template-columns:120px 1fr;gap:12px;border-top:1px solid var(--line);padding:10px 0}.shop dt{color:#8c8c92;font-size:12px}.shop dd{margin:0;overflow-wrap:anywhere}.shop-links{display:flex;gap:16px;flex-wrap:wrap}.shop-links a{color:var(--gold);text-underline-offset:4px}@media(max-width:760px){.shop-list{grid-template-columns:1fr}.shop dl div{grid-template-columns:1fr;gap:2px}}</style></head><body data-page-type="column" data-column-slug="perfume-shopping-kyoto" data-column-title="${esc(headline)}" data-column-category="travel"><script defer src="/assets/analytics.js"></script><header class="topbar"><a class="logo" href="/en/">Sillage</a><a class="language" href="/columns/kyoto-fragrance-shops" hreflang="ja" lang="ja">日本語の京都ガイド</a></header><main><header class="guide-hero"><div class="wrap"><p class="eyebrow">Kyoto fragrance guide</p><h1>${headline}</h1><p class="lede">${description}</p><div class="notice"><strong>Before you visit:</strong> opening hours and store services can change. Check the official page on the day of your visit. English assistance and tax-free shopping are marked “Not confirmed” unless verified by the store.</div><p>Last verified: ${dateEn(kyotoEnglish.lastVerified)}</p></div></header><section class="section"><div class="wrap"><p class="eyebrow">18 places to explore</p><div class="shop-list">${cards}</div></div></section></main><footer><a href="/en/">Sillage</a> · Japan Fragrance Guide</footer></body></html>`;
}
...
function renderJapaneseBrandsGuide() {
  ...
  const description = "An editorial introduction to four well-documented Japanese fragrance brands, with official links and practical notes for travelers shopping in Japan.";
  const brands = [
...
write(`${publicI18n}/kyoto-shops.en.json`, readFileSync("data/i18n/kyoto-shops.en.json", "utf8"));

console.log(`Generated English Phase 2: home, catalogue, ${localizedProducts.length} product pages, ${brandEntries.length} brand entries, ${brandEntries.filter((entry) => brandHasDetail(entry.key)).length} brand pages and 2 guides.`);

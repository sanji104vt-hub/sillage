...
- `data/site-copy.json` の `heroProductLine` に旧商品件数が固定されていた。公開トップは実データ件数を表示していたが、生成元文言として不整合が残っていた。
...
2. `data/site-copy.json` の旧固定件数を実データへ追従させる。
...
- 本番反映前に、英語5商品名・ノート英訳・京都記事不整合・サイトコピーの旧固定件数について最終レビューが必要。

...
import { storesForBrand } from "./store-data.mjs";
...
export function japanAvailability(product, brandSlug) {
  ...
  return {
    market: "JP",
    currency: "JPY",
    whereToTry: storesForBrand(brandSlug).map((store) => ({
      storeId: store.id,
      city: store.city,
      availabilityLevel: store.availabilityLevel,
    })),
    availabilityByCity: {
      tokyo: storesForBrand(brandSlug).some((store) => store.city === "tokyo") ? "brand-confirmed" : null,
      osaka: storesForBrand(brandSlug).some((store) => store.city === "osaka") ? "brand-confirmed" : null,
      kyoto: storesForBrand(brandSlug).some((store) => store.city === "kyoto") ? "brand-confirmed" : null,
    },
    officialPrices,
...
export function localizeProduct(product) {
  ...
  return {
    ...
    concentrationEn: product.concentration?.value
      ...
      : null,
    japanAvailability: japanAvailability(product, brand.slug),
